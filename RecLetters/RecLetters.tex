...
\item Sort your \EM~spreadsheet according to five sort keys: 1) Date
  you asked the staff to send the recommendation letters; 2) Method of
  recommendation (snail mail, \EJM, push email (for sorting purposes,
  ...
  sent); 3) Academic vs.\ Nonacademic; 4) Domestic vs.\ Foreign; 5)
  Employer Name.  (There should be a macro built into \EM~that can be
  executed to accomplish this sort -- see the instructions in
  \EM\texttt{-Instructions.xlsx}).  

\item After sorting the spreadsheet, execute the macro to ``hide'' the information that \JMStaff~does not need for purposes of tracking which letters have been sent.  (Instructions for how to execute this macro should be on the first page of \EM).  
...
\item You'll probably want next to immediately ``unhide'' those columns that were hidden, so that you can see them yourself.  Again, there should be a macro that accomplishes this described in the first-page instructions.
...
\item {\it After taking care of their push and pull email applications}, students should complete their \EJM~applications
  for this round (see below for details).
...
\item Recommenders who are not JHU faculty and who do NOT have their own
...
\begin{quote}
\begin{itemize}
\item[EJM] New inside recommenders should set \JMStaffEmail as their proxy (old inside recommenders should already have done this in prior years, and need to do nothing at all beyond sending letters to \JMStaffEmail)
...
summarize:
\begin{enumerate}
\item You will receive an \EM~spreadsheet from a student.  When you receive it, print it out.
...
get an account at \EJM.  According to Nina, your account is first established
...
So,
if you have a preferred JHU email address (JHED actually automatically

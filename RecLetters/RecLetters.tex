...
\begin{enumerate}
\item Snail mail (including ``Interfolio'')
...
\item Sort your \EM~spreadsheet according to five sort keys:
 ...
 \item Method of recommendation (snail mail, \EJM, push email (for sorting purposes,
  label it ``email-push''), pull email (label it ``email-pull''), \AJO, \AEA, or
...
\item {\it After taking care of their push and pull email applications}, students should complete their \EJM, \AJO, and \AEA~applications for this round (see below for details).
...
\item {\bf Snail Mail:}
\begin{enumerate}
\item \href{http://www.interfolio.com}{{\bf ``Interfolio.''}} \ifdvi\hypertarget{Interfolio}{}\fi This is a service that some employers use to outsource the processing of letters.  They accept snail mail letters, with the twist that each envelope containing a letter must also contain an ``authentication'' document that Interfolio uses to process the letter.  If an employer uses Interfolio, your first step should be to check with them whether they would be willing to use a ``pull email'' approach instead.  If not, then YOU (the job candidate) must therefore abide by the procedures for sending regular letters (detailed below) with the ADDITION that you must provide the printed-out authentication documents required for sending letters to Interfolio.  For further details, see the link at

\centerline{\url{http://www.interfolio.com}}
...
login ID at \EJM~will need to have an account created for them (the account is created automatically the first time a student identifies the recommender by giving \EJM~the recommender's email address), then they must upload their letters themselves.  
New \EJM~security measures prevent us from uploading letters on behalf of people
who are not JHU faculty.\footnote{The same steps that the
    staff completes in the bullet point about JHU faculty.  They
    log in, they upload their letter, they indicate it's for, e.g.,
    \texttt{colleen.carey@jhu.edu}, and then they select all employers and tick the box for all
    future employers.}  The student must communicate this information to
  the recommender.
...
\item {\bf \AJO ~ (\AJOLink):}
Students will enter only the Department of Economics faculty letter writer's name, each with a 
different and unique email address (use the faculty's JHU email address), on the cover sheet
provided by the AJO system.
To complete this, each Econ faculty member will in turn need to login to enter
Maggie Potts as their proxy,  using the \JMStaffEmail~email address.


\item {\bf \AEA ~ (\AEALink):}
Students who are using \AEA~must inform their JHU Economics Department reference writer to go to \AEARecLink~ and set up a surrogate for their reference letters. The surrogate name is Maggie Potts and the email address is \JMStaffEmail. This service will be provided for JHU Economic professors only.
...
\begin{quote}
\begin{itemize}
\item[EJM] New inside recommenders should set \JMStaffEmail~as their proxy (old inside recommenders should already have done this in prior years, and need to do nothing at all beyond sending letters to \JMStaffEmail). Maggie can only proxy JHU Economics Faculty letters.
\item[AJO] New inside recommenders should set \JMStaffEmail~as their proxy (old inside recommenders should already have done this in prior years, and need to do nothing at all beyond sending letters to \JMStaffEmail). Maggie can only proxy JHU Economics Faculty letters.
\item[AEA/JOE] All inside recommenders should set \JMStaffEmail~as their proxy. The faculty need to set Maggie Potts as their proxy each year, as this system does not carry their proxies over from year to year. Maggie can only proxy JHU Economics Faculty letters.
